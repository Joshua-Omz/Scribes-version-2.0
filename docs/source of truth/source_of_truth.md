...
| **No commercial layer of any kind** | Scribes has no subscriptions, no tiers, no payment processing, no premium features. Every registered user has identical access to every platform capability. A single fair-use allowance on image and sound uploads exists solely to prevent storage abuse — it applies uniformly to every user, is not purchasable, and is not a product tier. This is a permanent architectural and product decision, not a placeholder for future monetization. See `scribes_media_contracts_v2.md`. |
...
│   ├── server/router.go            — Chi setup, mounts every feature's routes
...
**This is the original v1 surface — 43 endpoints across 9 sprints.** It has since been extended by feature additions covered in dedicated contract documents. The current full surface, including all extensions, is **70 endpoints**:

```
MEDIA (v2, no billing)   POST   /media/upload           ← scribes_media_contracts_v2.md
                         GET    /media/allowance

TAG                      GET    /tags/suggest            ← PUBLIC · scribes_search_recommendations_v2.md
...
RECOMMENDATION           GET    /recommendations/most-insightful   ← PUBLIC
                         GET    /recommendations/most-affirmed     ← PUBLIC
                         GET    /recommendations/prophetic         ← PUBLIC
                         GET    /recommendations/trending          ← PUBLIC
                         GET    /posts/:id/similar                 ← PUBLIC
                         GET    /recommendations/for-you            (auth required)

BIBLE                    GET    /bible/books                       ← PUBLIC · scribes_bible_drawer_contract.md
                         GET    /bible/:book/:chapter               ← PUBLIC
                         GET    /bible/:book/:chapter/:verse_start-:verse_end  ← PUBLIC
                         GET    /bible/search                       ← PUBLIC
                         GET    /bible/reading-position               (auth required)
                         POST   /bible/reading-position               (auth required)
```

**Rule:** any endpoint marked PUBLIC must work with zero `Authorization` header. This is not optional — it's the technical backbone of the outward evangelism principle. Note that the media, search, tag, recommendation, and Bible surfaces are overwhelmingly public — consistent with the platform having no commercial layer and no reason to gate discovery or reading behind authentication.
