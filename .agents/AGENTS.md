...
* **Cache-First Feed & Post Detail:** Remote feed responses must always seed the local Drift SQLite `posts` table in the background. Post detail screens (`/posts/:id`) must employ a cache-first read path—resolving from the local database immediately so content renders without network latency or offline failures.
...
* **Offline Session Preservation (Auth):** Never invoke `logout()` or delete stored credentials when network requests fail due to socket errors, timeouts, or offline connectivity. Session invalidation is strictly reserved for HTTP 401 Unauthorized or 403 Forbidden responses from the server. When offline, always restore the cached `User` profile to keep the user authenticated.
